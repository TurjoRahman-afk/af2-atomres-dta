"""
AF2-PocketCross-DTA — attention-pooling variant
================================================
Identical to model_pocketcross_gnnprior.py (the current champion: cold-protein seed 42
test MSE 0.3519 / CI 0.8547 / r2m 0.4928) with EXACTLY ONE change:

  the two sequence summary vectors (gd, gp) are now produced by KANPM's learned
  LinearAttention pooling instead of a plain masked mean.

Motivation: KANPM's own ablation prices "W/O Linear Attention" at +0.054 MSE on the
unseen-protein split — the second most valuable component after the sequence branch, and
~4x the graph branch (+0.014). KANPM has this component; this model previously did not.
Averaging 1200 residues equally dilutes the ~10-30 binding-site residues by ~40x; a
learned weighted pool can focus on them instead.

LinearAttention is copied verbatim from KANPM (input 128, hidden 64, 8 heads) so the
comparison is faithful to the component their ablation measured.

Everything else — encoders, transformers, both GNNs, PocketPrior, StructGuidedInteraction,
gated graph fusion, KAN [512,1024,512,1] — is unchanged. Output shapes are unchanged
([B,128] each), so the KAN input stays 512.
"""
import os
import torch
...
POOL_HEADS = 8   # KANPM uses LinearAttention(128, 64, 8)
...
class LinearAttention(nn.Module):
    """KANPM's learned attention pooling — scores every position, softmaxes into weights,
    and takes a weighted sum instead of a flat average. Averaged over `heads` heads so it
    can attend to several regions at once.
    masks: [B, heads, L] with 1 = real position, 0 = padding."""
    def __init__(self, input_dim=128, hidden_dim=64, heads=8):
        super().__init__()
    ...
    def forward(self, x, masks):
        a = torch.tanh(self.linear_first(x))                          # [B, L, hidden]
        a = self.linear_second(a)                                     # [B, L, heads]
        a = a.transpose(1, 2)                                         # [B, heads, L]
        a = torch.where(masks > 0.5, a, -9e15 * torch.ones_like(a))   # exclude padding
        att = self.softmax(a)                                         # [B, heads, L]
        emb = att @ x                                                 # [B, heads, D]
        return torch.sum(emb, 1) / self.heads                         # [B, D]
...
class ProteinGraphNet(nn.Module):
    """Exposes its per-residue embeddings (densified + aligned to the +1 BOS offset)
    alongside the pooled protein-level vector — the change that won at 0.3519."""
...
class PocketPrior(nn.Module):
    """Per-residue bindability score from ProteinGraphNet's learned per-residue embedding."""
    ...
    def forward(self, residue_emb):          # [B, Lp, in_dim]
        return self.mlp(residue_emb).squeeze(-1)   # [B, Lp]
...
class StructGuidedInteraction(nn.Module):
    ...
    def forward(self, Hd, Hp, drug_mask, prot_mask, pocket_logit):
        ...
        scores = scores + self.beta * pocket_logit.unsqueeze(1)
        res_pad = (prot_mask < 0.5).unsqueeze(1)
        scores = scores.masked_fill(res_pad, -9e15)
        I = self.drop(torch.softmax(scores, dim=-1))                              # [B,Nd,Lp]
        ctx = torch.bmm(I, Hp)                                                     # [B,Nd,D]
        inter = Hd * ctx                                                          # [B,Nd,D]
        am = drug_mask.unsqueeze(-1)
        f_int = (inter * am).sum(dim=1) / am.sum(dim=1).clamp(min=1)              # [B,D]
        return f_int, I
...
class MODEL(nn.Module):
    def __init__(self, hp, device):
        ...
        # THE CHANGE: learned attention pooling for the sequence summaries (was plain mean)
        self.pool_heads = POOL_HEADS
        self.drug_attn = LinearAttention(d, 64, POOL_HEADS)
        self.target_attn = LinearAttention(d, 64, POOL_HEADS)
    ...
    def forward(self, drug_mat, drug_mask, prot_mat, prot_mask, drug_graph, protein_graph):
        ...
        # learned attention pooling (was: plain masked mean over all positions)
        h = self.pool_heads
        gd = self.drug_attn(Hd, drug_mask.unsqueeze(1).expand(-1, h, -1))   # [B,128]
        gp = self.target_attn(Hp, prot_mask.unsqueeze(1).expand(-1, h, -1))  # [B,128]
